...
- **Upgrading from `0.2.2`:** rerun `init` or `install` with version `0.4.0` so
  review outcome synchronization recognizes the repository identity returned
  by the `gh` CLI for linked issues.
- **Upgrading from `0.2.1`:** rerun `init` or `install` with version `0.4.0` to
...
- **Upgrading from `0.2.0`:** stop any old `npx ... run build|review`
  processes. Version `0.4.0` ignores their `.git/gsd-loop` state; remove that
