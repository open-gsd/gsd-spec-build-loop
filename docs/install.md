...
Use the `@latest` commands above for upgrades.

- **Upgrading from `0.4.0`:** rerun `init` or `install` to bundle the outcome
  helper required by discovery and specification.
- **Upgrading from `0.3.0`:** rerun `init` or `install` to refresh the installed
  skills and deterministic helpers.
- **Upgrading from `0.2.5`:** rerun `init` or `install` to
  add the interactive discovery workflow and its deterministic map helpers.
- **Upgrading from `0.2.3` or `0.2.4`:** rerun `init` to install discovery and
  the corrected Claude skill entry points. Upgrades from `0.2.3` also install
  the build linkage guard and Grok adapter, and locally exclude
  `.gsd/scheduled_tasks.lock`. Use `install` instead only when repository setup
  is not wanted.
- **Upgrading from `0.2.2`:** rerun `init` or `install` so review outcome
  synchronization recognizes the repository identity returned by the `gh` CLI
  for linked issues.
- **Upgrading from `0.2.1`:** rerun `init` or `install` to add the native Cursor
  and Gemini adapters. Existing canonical and Claude installs remain
  installer-owned and update safely.
- **Upgrading from `0.2.0`:** stop any old `npx ... run build|review`
  processes. Current releases ignore their `.git/gsd-loop` state; remove that
