...
EXPECTED_NPM=$(printf 'ci\nview @opengsd/gsd-loop@0.3.0 version gitHead --json\npublish')
test "$(cat "$TEST_ROOT/published.log")" = "$EXPECTED_NPM"
test "$(cat "$TEST_ROOT/release-state")" = v0.3.0
test "$(cat "$TEST_ROOT/release-commit-state")" = "$CURRENT_SHA"
grep -q "^release create v0.3.0 .*--target $CURRENT_SHA .*--generate-notes" "$TEST_ROOT/github.log"
