...
export function transformOutcomeChecklist(body, state) {
  ...
  const seen = new Set();
  let count = 0;
  let inOutcomeDescription = false;
  for (let index = sectionStart + 1; index < sectionEnd; index += 1) {
    ...
    if (!match) {
      const malformed =
        ...
        (!inOutcomeDescription ? line.match(INDENTED_OUTCOME_REFERENCE) : null);
      ...
      if (line && (!/^\s/.test(line) || LIST_ENTRY.test(line))) {
        inOutcomeDescription = false;
      }
      continue;
    }
