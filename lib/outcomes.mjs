...
export function transformOutcomeChecklist(body, state) {
  ...
  const seen = new Set();
  let count = 0;
  let outcomeContinuationIndent = null;
  for (let index = sectionStart + 1; index < sectionEnd; index += 1) {
    ...
    if (!match) {
      ...
      const isOutcomeContinuation =
        outcomeContinuationIndent !== null &&
        indentation >= outcomeContinuationIndent;
      const malformed =
    ...
    seen.add(outcome);
    count += 1;
    outcomeContinuationIndent = prefix.length - prefix.trimStart().length + 2;
