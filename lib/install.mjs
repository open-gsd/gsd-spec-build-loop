...
export const SUPPORTED_AGENTS = new Set(["codex", "claude", "cursor", "gemini", "grok", "kimi"]);
...
const ADAPTER_ROOTS = new Map([
  ...
  ["grok", { label: "Grok", path: [".grok", "skills"] }],
]);
...
function blockingDirectoryComponent(path) {
  const components = [];
  let current = resolve(path);
  while (true) {
  ...
  for (const component of components.reverse()) {
    ...
    try {
      if (!statSync(component).isDirectory()) {
        return component;
      }
    } catch (error) {
      if (error.code === "ENOENT") {
...
function stageSkill(sourceRoot, destination, skill) {
  ...
  try {
    ...
    if (["discover", "spec", "build", "review"].includes(lane)) {
      cpSync(join(sourceRoot, "loop", `${lane}.md`), join(stage, "playbook.md"));
      if (["discover", "spec"].includes(lane)) {
        const runtime = join(stage, "scripts", "runtime");
        mkdirSync(runtime, { recursive: true });
        for (const module of ["discovery-map.mjs", "errors.mjs"]) {
          cpSync(join(sourceRoot, "lib", module), join(runtime, module));
        }
      } else if (lane === "build") {
        const runtime = join(stage, "scripts", "runtime");
        mkdirSync(runtime, { recursive: true });
        for (const module of ["errors.mjs", "linkage.mjs", "process.mjs"]) {
          cpSync(join(sourceRoot, "lib", module), join(runtime, module));
        }
      } else if (lane === "review") {
        const runtime = join(stage, "scripts", "runtime");
...
function preflightAdapterRoot(canonicalRoot, adapterRoot, adapterMode) {
  const conflicts = [];
  const blockingComponent = blockingDirectoryComponent(adapterRoot);
...
function preflight(canonicalRoot, adapterRoots, adapterMode) {
  const conflicts = [];
  const blockingComponent = blockingDirectoryComponent(canonicalRoot);
  if (blockingComponent) {
    conflicts.push(blockingComponent);
  } else {
    for (const skill of SKILLS) {
