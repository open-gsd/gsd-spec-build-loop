...
function parseQueueIssues(value) {
  ...
  return lines.map((line) => {
    const match = line.match(/^- (S-[1-9]\d*) — \[([^\]]+)\]\((https:\/\/[^)]+\/issues\/([1-9]\d*))\)$/);
    if (!match) throw new BlockedError("Queue issues entries must use the exact S-N linked form");
    if (ids.has(match[1])) throw new BlockedError(`Queue issues repeats ${match[1]}`);
    ids.add(match[1]);
    return { id: match[1], title: match[2], url: match[3], issue: Number(match[4]) };
  });
}
