...
function parseDecisionFrontier(value, allowNotReady) {
  ...
  return headings.map((heading, index) => {
    ...
    const issueNumber = issue === "Pending." ? null : Number(issue.slice(1));
    if (issueNumber !== null && issueNumbers.has(issueNumber)) {
      throw new BlockedError(`Decision frontier repeats decision issue #${issueNumber}`);
    }
    if (issueNumber !== null) issueNumbers.add(issueNumber);
    return {
      id: heading[1],
      title,
      type,
...
function parseQueueIssues(value) {
  if (value === "None.") return [];
  if (!value) {
    throw new BlockedError("Queue issues must be None. or use the exact S-N linked form");
  }
  const lines = value.split(/\r?\n/).filter(Boolean);
  const ids = new Set();
  return lines.map((line) => {
    const linked = line.match(/^- (S-[1-9]\d*) — \[([^\]]+)\]\((https:\/\/[^)]+\/issues\/([1-9]\d*))\)$/);
    if (!linked) throw new BlockedError("Queue issues entries must use the exact S-N linked form");
    const id = linked[1];
...
function parseSlices(value, allowNotReady) {
  ...
  return headings.map((heading, index) => {
    ...
    const title = heading[3].trim();
    if (!title || /[\[\]()]/.test(title)) {
      throw new BlockedError(`${heading[1]} must have a filing-compatible title`);
    }
    if (titles.has(title)) {
...
export function validateDiscoveryMap(body, { allowNotReady = false } = {}) {
  ...
  if (!sections["Out of scope"]) {
    throw new BlockedError("Out of scope must be None. or an explicit boundary");
  }
  if (!allowNotReady) {
