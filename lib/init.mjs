...
import { agentArguments, RUNNER_AGENTS, stateDirectory } from "./runner.mjs";
...
function checked(program, argumentsList, { cwd, run = runProcess, input: commandInput } = {}) {
  const result = run(program, argumentsList, { cwd, input: commandInput });
  if (result.status !== 0) {
    throw new CliError(`${program} ${argumentsList.join(" ")} failed: ${result.stderr.trim() || result.stdout.trim() || `exit ${result.status}`}`);
  }
...
function successfulChecks(cwd, repo, branch, run) {
  ...
  const pullRequests = run("gh", ["pr", "list", "--repo", repo, "--state", "open", "--limit", "1", "--json", "headRefOid,updatedAt"], { cwd });
  if (pullRequests.status === 0) {
    const [latest] = parseJson(pullRequests.stdout, "open GitHub pull requests");
    if (latest?.headRefOid) references.push(latest.headRefOid);
  }
  const checkRuns = references.flatMap((reference) => {
    const response = run("gh", ["api", `repos/${repo}/commits/${reference}/check-runs`], { cwd });
    if (response.status !== 0) return [];
    return parseJson(response.stdout, `GitHub check runs for ${reference}`).check_runs ?? [];
  });
  const seen = new Set();
  return checkRuns
...
function writeLocalState(cwd, repo, agent, check, run) {
  ...
  const config = {
    ...
    requiredCheck: check,
    intervals: { workMinutes: 15, idleMinutes: 60, idleLimit: 3 },
  };
...
export async function initialize({ sourceRoot, cwd, options, run = runProcess, io: suppliedIo }) {
  ...
  try {
    ...
    if (!root) {
      ...
      repo = resolveRepository({ cwd: root, repo, run });
    }
