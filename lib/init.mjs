...
export async function initialize({ sourceRoot, cwd, options, run = runProcess, io: suppliedIo }) {
  ...
  try {
    ...
    if (!root) {
      ...
      let checkoutRepo;
      try {
        checkoutRepo = resolveRepository({ cwd: root, run });
      } catch (error) {
        if (!/no git remotes found/i.test(error.message)) throw error;
        throw new CliError(
          "no GitHub repository is connected to this checkout; run "
          + "`npx @opengsd/gsd-loop@latest install` to install the skills without GitHub, "
          + "then add or create a GitHub remote and rerun `init`",
        );
      }
