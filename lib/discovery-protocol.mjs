...
import { transformOutcomeChecklist } from "./outcomes.mjs";
...
function repositoryIssues(options, { refresh = false } = {}) {
  if (!refresh && options.issueSnapshot) return options.issueSnapshot;
  options.issueSnapshot = flattenPages(checked("gh", [
    "api", "--paginate", "--slurp",
    "-H", "X-GitHub-Api-Version: 2026-03-10",
    `repos/${options.repo}/issues?state=all&per_page=100`,
  ], options), "repository issues").filter((issue) => !issue.pull_request);
  return options.issueSnapshot;
}
...
function replaceSection(body, name, value) {
  ...
  if (index === -1) throw new BlockedError(`discovery map is missing its ${name} section`);
  const heading = headings[index];
  const nextHeading = headings[index + 1];
  const end = nextHeading?.index ?? body.length;
  const newline = body.includes("\r\n") ? "\r\n" : "\n";
  const separator = nextHeading ? `${newline}${newline}` : "";
  return `${body.slice(0, heading.index)}## ${name}${newline}${newline}${value}${separator}${body.slice(end)}`;
}
...
export function graduateDiscoveryMap({ cwd, repo, map, mapBody, run = runProcess }) {
  ...
  repositoryIssues(options, { refresh: true });
...
const ACTIVE_QUEUE_VALIDATION = "active";
const TERMINAL_RECOVERY_QUEUE_VALIDATION = "terminal-recovery";

function allowsTerminalRecovery(validationMode) {
  if (validationMode === ACTIVE_QUEUE_VALIDATION) return false;
  if (validationMode === TERMINAL_RECOVERY_QUEUE_VALIDATION) return true;
  throw new TypeError(`unknown linked queue validation mode: ${validationMode}`);
}

function validateSliceContractShape(draft, { allowCompletedOutcomes = false } = {}) {
  const headings = [...draft.matchAll(/^## ([^\r\n]+)\r?$/gm)];
  if (
    JSON.stringify(headings.map((heading) => heading[1]))
    !== JSON.stringify(SLICE_CONTRACT_SECTIONS)
  ) {
    throw new BlockedError(
      `slice issue must contain exactly these sections in order: ${SLICE_CONTRACT_SECTIONS.join(", ")}`,
    );
  }
  const sections = Object.fromEntries(headings.map((heading, index) => {
    const start = heading.index + heading[0].length;
    const end = headings[index + 1]?.index ?? draft.length;
    return [heading[1], draft.slice(start, end).trim()];
  }));
  const why = sections.Why.split(/\r?\n/).filter((line) => {
    const value = line.trim();
    return value
      && !/^Discovery (?:map|slice|plan):/.test(value)
      && !/^Needs #[1-9]\d* merged$/.test(value);
  });
  if (!why.length) throw new BlockedError("slice issue Why section has no rationale");
  const normalizedDraft = draft.replace(/\r\n/g, "\n");
  const pendingOutcomes = transformOutcomeChecklist(normalizedDraft, "pending");
  const completedOutcomes = allowCompletedOutcomes
    ? transformOutcomeChecklist(normalizedDraft, "complete")
    : null;
  if (pendingOutcomes !== normalizedDraft && completedOutcomes !== normalizedDraft) {
    const expected = allowCompletedOutcomes
      ? "consistently pending or completed checkboxes"
      : "pending checkboxes";
    throw new BlockedError(`slice issue Outcomes must use ${expected}`);
  }
  for (const outcome of sections.Outcomes.matchAll(
    /^\s*-\s+\[[ xX]\]\s+(O-\d+)\b(.*)$/gm,
  )) {
    if (!/^\s+—\s+\S/.test(outcome[2])) {
      throw new BlockedError(`slice issue outcome ${outcome[1]} has no description`);
    }
  }
  const exclusions = [];
  for (const line of sections.Exclusions.split(/\r?\n/).filter((entry) => entry.trim())) {
    const match = line.match(/^- (X-[1-9]\d*) — \S/);
    if (match) {
      exclusions.push(match[1]);
    } else if (!exclusions.length || !/^[ \t]+\S/.test(line)) {
      throw new BlockedError("slice issue has a malformed exclusion");
    }
  }
  if (!exclusions.length) {
    throw new BlockedError("slice issue Exclusions section has no X-N entries");
  }
  if (new Set(exclusions).size !== exclusions.length) {
    throw new BlockedError("slice issue has duplicate exclusion identifiers");
  }
  if (!/^\s*[-+*]\s+\S/m.test(sections["Code pointers"])) {
    throw new BlockedError("slice issue Code pointers section has no entries");
  }
  if (!/^\s*[-+*]\s+\S/m.test(sections["Testing notes"])) {
    throw new BlockedError("slice issue Testing notes section has no entries");
  }
  if (!/^\s*1[.)]\s+\S/m.test(sections["Manual walkthrough"])) {
    throw new BlockedError("slice issue Manual walkthrough has no numbered steps");
  }
}
...
function validateSliceContract({
  ...
  validateSliceContractShape(draft, { allowCompletedOutcomes });
}
...
function assertQueueIssueState(issue, { allowClosed = false, allowReady = false } = {}) {
  const state = String(issue.state).toLowerCase();
  if (state !== "open" && !(allowClosed && state === "closed")) {
    throw new BlockedError(
...
function validateLinkedQueue(
  parsed,
  issues,
  repo,
  map,
  options,
  validationMode = ACTIVE_QUEUE_VALIDATION,
) {
  const terminalRecovery = allowsTerminalRecovery(validationMode);
  ...
  for (const entry of parsed.queueIssues) {
    ...
    assertQueueIssueState(issue, {
      allowClosed: terminalRecovery,
      allowReady: terminalRecovery,
    });
    ...
    validateSliceContract({
      ...
      allowCompletedOutcomes: terminalRecovery,
    });
...
function reconcileDiscoverySlicesWithOptions(options) {
  const { repo, map } = options;
  const currentMap = mapIssue(options);
...
export function reconcileDiscoverySlices({ cwd, repo, map, run = runProcess }) {
  return reconcileDiscoverySlicesWithOptions({ cwd, repo, map, run });
}

export function fileDiscoverySlice({ cwd, repo, map, slice, title, bodyPath, run = runProcess }) {
  const options = { cwd, repo, map, run };
  const recovery = reconcileDiscoverySlicesWithOptions(options);
  ...
  if (recovery.approvalRequired) repositoryIssues(options, { refresh: true });
  ...
  if (!matches.length) {
    ...
    matches = matchingSliceIssues(repositoryIssues(options, { refresh: true }), map, slice);
  }
  if (matches.length !== 1) {
  ...
  const verifiedLinked = validateLinkedQueue(
    verified,
...
export function completeDiscoveryMap({ cwd, repo, map, run = runProcess }) {
  ...
  const linked = validateLinkedQueue(
    ...
    map,
    options,
    closed ? TERMINAL_RECOVERY_QUEUE_VALIDATION : ACTIVE_QUEUE_VALIDATION,
  );
