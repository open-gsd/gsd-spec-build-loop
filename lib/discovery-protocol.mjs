...
function validateLinkedQueue(
  ...
  map,
  options,
  { allowClosed = false, allowCompletedOutcomes = false, allowReady = false } = {},
) {
  ...
  for (const entry of parsed.queueIssues) {
    ...
    assertQueueIssueState(issue, { allowClosed, allowReady });
...
export function completeDiscoveryMap({ cwd, repo, map, run = runProcess }) {
  ...
  const linked = validateLinkedQueue(
    ...
    map,
    options,
    { allowClosed: closed, allowCompletedOutcomes: closed, allowReady: closed },
  );
