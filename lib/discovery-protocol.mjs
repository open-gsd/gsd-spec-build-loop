...
export function graduateDiscoveryMap({ cwd, repo, map, mapBody, run = runProcess }) {
  ...
  if (
    currentGraduation === "Ready for `gsd-loop-spec`."
    && JSON.stringify(current.slices) !== JSON.stringify(proposed.slices)
  ) {
    throw new BlockedError(
      "ready delivery slices are immutable; return the map to Not ready. before changing them",
    );
  }
...
function assertQueueIssueOpen(issue) {
...
export function completeDiscoveryMap({ cwd, repo, map, run = runProcess }) {
  ...
  const currentMap = mapIssue(options);
  const parsed = validateDiscoveryMap(currentMap.body);
  const linked = validateLinkedQueue(parsed, repositoryIssues(options), repo, map, options);
  ...
  checked("gh", ["issue", "close", String(map), "--repo", repo], options);
