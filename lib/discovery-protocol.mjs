...
export function reconcileDiscoverySlices({ cwd, repo, map, run = runProcess }) {
  ...
  const recovered = [];
  let foundGap = false;
  for (const declared of parsed.slices.slice(entries.length)) {
    ...
    if (foundGap) {
      throw new BlockedError(`${declared.id} exists before an earlier delivery slice`);
    }
    const issue = matches[0];
  ...
  return {
    recovered,
    missing: parsed.slices.slice(verified.queueIssues.length).map((slice) => slice.id),
  };
}
...
export function fileDiscoverySlice({ cwd, repo, map, slice, title, bodyPath, run = runProcess }) {
  ...
  const verified = validateDiscoveryMap(mapIssue(options).body);
  const verifiedLinked = validateLinkedQueue(verified, repositoryIssues(options), repo, map);
