...
function validateSliceContractShape(draft) {
  ...
  if (transformOutcomeChecklist(normalizedDraft, "pending") !== normalizedDraft) {
    throw new BlockedError("slice issue Outcomes must use pending checkboxes");
  }
  const exclusions = [];
...
function validateSliceContract({
  ...
  validateSliceContractShape(draft);
}
...
function assertQueueIssueOpen(issue, { allowClosed = false, allowReady = false } = {}) {
  const state = String(issue.state).toLowerCase();
  if (state !== "open" && !(allowClosed && state === "closed")) {
    throw new BlockedError(
...
function validateLinkedQueue(
  ...
  map,
  options,
  { allowClosed = false, allowReady = false } = {},
) {
  ...
  for (const entry of parsed.queueIssues) {
    ...
    assertQueueIssueOpen(issue, { allowClosed, allowReady });
...
export function completeDiscoveryMap({ cwd, repo, map, run = runProcess }) {
  ...
  const linked = validateLinkedQueue(
    ...
    map,
    options,
    { allowClosed: closed, allowReady: closed },
  );
