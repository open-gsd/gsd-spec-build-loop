...
export function graduateDiscoveryMap({ cwd, repo, map, mapBody, run = runProcess }) {
  ...
  if (
    ...
    && (current.queueIssues.length || hasFiledSliceIssue(options))
  ) {
    throw new BlockedError(
...
function hasFiledSliceIssue(options) {
  return repositoryIssues(options).some((issue) => sliceMarkers(issue.body ?? "")?.map === options.map);
}
