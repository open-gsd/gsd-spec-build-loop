...
function assertQueueIssueState(issue, options) {
  ...
  for (const pullRequest of closure.closedByPullRequestsReferences ?? []) {
    ...
    if (
      String(evidence.state).toLowerCase() === "merged"
      && evidence.mergedAt
      && labelNames(evidence.labels).includes("gsd:approved")
    ) {
      return;
    }
  }
  throw new BlockedError(`queue issue #${issue.number} closed without an approved merged pull request`);
}
