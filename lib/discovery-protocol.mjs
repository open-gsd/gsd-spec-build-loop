...
export function validateDecisionEvidence({ cwd, repo, map, mapBody, run = runProcess }) {
  ...
  assertSlicePlanTransition(currentMap.body, validatedBody, options);
...
function slicePlanApproval(map, currentBody, proposedBody) {
  const from = slicePlanDigest(currentBody);
  const to = slicePlanDigest(proposedBody);
  if (from === to) return null;
  return `gsd-loop slice-plan approval for map #${map}

From: sha256:${from}
To: sha256:${to}`;
}

function assertSlicePlanTransition(currentBody, proposedBody, options) {
  const approval = slicePlanApproval(options.map, currentBody, proposedBody);
  if (!approval) return false;
  const matches = issueComments(options, options.map, `map #${options.map} comments`).filter((comment) => (
    comment.body?.trim() === approval
    && TRUSTED_ASSOCIATIONS.has(comment.author_association)
    && comment.user?.type === "User"
  ));
  if (matches.length !== 1) {
    throw new BlockedError(`slice-plan approval requires exactly one trusted human comment:\n${approval}`);
  }
  return true;
}

export function validateSlicePlanTransition({ cwd, repo, map, mapBody, run = runProcess }) {
  const options = { cwd, repo, map, run };
  return assertSlicePlanTransition(mapIssue(options).body, mapBody, options);
}
...
function validateSliceContract({ draft, map, slice, expectedDependencies }) {
...
function assertQueueIssueState(issue, options) {
  const state = String(issue.state).toLowerCase();
  if (state === "open") return;
  if (state !== "closed") {
    throw new BlockedError(`queue issue #${issue.number} has an unknown state`);
  }
  const closure = parseJson(checked("gh", [
    "issue", "view", String(issue.number), "--repo", options.repo,
    "--json", "state,closedByPullRequestsReferences",
  ], options), `queue issue #${issue.number} closure`);
  const reviewer = checked("gh", ["api", "user", "--jq", ".login"], options);
  for (const pullRequest of closure.closedByPullRequestsReferences ?? []) {
    const evidence = parseJson(checked("gh", [
      "pr", "view", String(pullRequest.number), "--repo", options.repo,
      "--json", "state,mergedAt,headRefOid",
    ], options), `closing pull request #${pullRequest.number}`);
    if (String(evidence.state).toLowerCase() !== "merged" || !evidence.mergedAt || !evidence.headRefOid) continue;
    const verdict = `gsd-loop verdict for ${evidence.headRefOid} issue #${issue.number}`;
    const comments = issueComments(
      options,
      pullRequest.number,
      `closing pull request #${pullRequest.number} comments`,
    );
  ...
  throw new BlockedError(
    `queue issue #${issue.number} closed without a trusted reviewer verdict for its merged closing pull request`,
  );
}

function validateLinkedQueue(parsed, issues, repo, map, options) {
  ...
  for (const entry of parsed.queueIssues) {
    ...
    assertQueueIssueState(issue, options);
...
export function reconcileDiscoverySlices({ cwd, repo, map, run = runProcess }) {
  ...
  if (!candidate?.issue) {
    return { approvalRequired: null, recovered: [], missing: unlinked.map((slice) => slice.id) };
  }
...
export function runDiscoveryProtocol(parsed, { cwd = process.cwd(), run = runProcess } = {}) {
  ...
  if (parsed.command === "validate-plan") {
    if (!parsed.bodyPath) throw new UsageError("validate-plan requires --body-file");
    return {
      approved: validateSlicePlanTransition({
        ...common,
