...
issue from ready to open-PR. `/loop` provides the repetition; this skill
provides the unit.

Pair it with `/loop /gsd-loop-review` in a second session. Both can share a
GitHub token — the reviewer never touches git, so there's nothing to race.
