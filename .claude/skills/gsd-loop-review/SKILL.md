...
Each pass audits one PR, posts one verdict, sets labels, stops. `/loop`
supplies the cadence.

Runs alongside `/loop /gsd-loop-build` in its own session. Sharing a token is
fine — this skill's entire output is comments and labels, so it can't collide
with the builder's git state.
