...
This repository ships four agent-neutral playbooks under `loop/`. They work
...
| Ask | Playbook | Mode |
|---|---|---|
| "map this effort", "resolve this discovery map" | `loop/discover.md` | interactive — one chart, decision, or recovery/graduation pass |
...
- Never merge, never enable auto-merge, never force-push. Humans own every
  merge.
- A queue issue is the whole product and scope contract: implement only its
  `O-N` outcomes, treat `X-N` exclusions as binding, and escalate with
...
| Harness | Discover | Spec | Build | Review | Schedule |
|---|---|---|---|---|---|
...
Discovery and spec require a human and may share a session. Run build and
review in separate harness sessions. Each direct invocation is one pass.
